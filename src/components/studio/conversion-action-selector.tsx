...
export function ConversionActionSelector({
  ...
  return (
    <div className="space-y-3 pt-4 border-t border-ivory-border/60">
        ...
        <button
          ...
          className={`flex items-center justify-center gap-1.5 px-3 py-2.5 rounded-xl text-xs font-medium border transition-all ${
            actionType === 'NONE'
              ...
              : 'bg-white text-ink border-ivory-border hover:border-terracotta/40'
          }`}
        >
      ...
      {actionType !== 'NONE' && (
        <div className="p-3.5 bg-ivory-card border border-ivory-border rounded-xl space-y-2">
          {errorMessage && (
          ...
          {effectiveDestination && !isEditing ? (
            <div className="flex items-center justify-between gap-3">
              <div className="text-xs text-ink space-y-0.5">
                ...
                {actionIsOverride && (
                  <span className="inline-block ml-1.5 px-1.5 py-0.5 bg-terracotta-light text-terracotta text-[10px] rounded font-medium">
              ...
              <button
                ...
                className="inline-flex items-center gap-1 px-2.5 py-1 text-xs font-medium text-ink bg-white border border-ivory-border rounded-lg hover:border-terracotta/40 transition-colors shrink-0"
              >
            ...
            <form onSubmit={handleSaveMissingForBusiness} className="space-y-2.5">
              ...
              <div className="flex flex-col gap-2">
                <input
                  ...
                  required
                  className="w-full px-3 py-2.5 bg-white border border-ivory-border rounded-xl text-xs text-ink placeholder:text-ink-muted/50 focus:outline-none focus:ring-2 focus:ring-terracotta/30 focus:border-terracotta transition-all"
                />

                <button
                  type="submit"
                  disabled={isPending || !editValue.trim()}
                  className="w-full inline-flex items-center justify-center gap-1.5 px-3.5 py-2.5 bg-terracotta text-white rounded-xl text-xs font-medium hover:bg-terracotta-dark disabled:opacity-50 transition-colors shadow-2xs"
                >
                  {isPending ? (
            ...
            <div className="space-y-3 pt-1">
              ...
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 pt-1">
                <button
                  ...
                  disabled={isPending || !editValue.trim()}
                  className="w-full sm:w-auto px-3 py-2 bg-white border border-ivory-border hover:border-terracotta/50 text-ink rounded-xl text-xs font-medium transition-colors text-center"
                >
                  Pour cette publication
                </button>
                ...
                <button
                  ...
                  disabled={isPending || !editValue.trim()}
                  className="w-full sm:w-auto inline-flex items-center justify-center gap-1 px-3 py-2 bg-terracotta text-white rounded-xl text-xs font-medium hover:bg-terracotta-dark disabled:opacity-50 transition-colors text-center"
                >
