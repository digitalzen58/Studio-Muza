    ...
    <Card variant="default" className="flex flex-col justify-between gap-4 p-4 sm:p-5 transition-all hover:shadow-md border-ivory-border/80 h-full">
      ...
      {whyNow && (
        <div className="bg-terracotta-light/60 px-3 py-2.5 rounded-xl border border-terracotta-border/30 flex items-start gap-2">
          ...
          <p className="text-xs text-terracotta-dark font-medium leading-relaxed">
            <strong className="font-semibold text-primary">Pourquoi maintenant ?</strong> {whyNow}
          </p>
        </div>
      )}
      ...
      {recommendation.assetGuidance?.captureBrief &&
        (readiness === 'PARTIAL' || readiness === 'MISSING') && (
          <div className="bg-cream-subtle border border-cream-border/90 px-3.5 py-2.5 rounded-xl flex items-start gap-2.5">
            <span className="text-primary text-xs font-serif font-bold mt-0.5 shrink-0">✦</span>
      ...
      {(requiresCamera || requiresVoiceover) && (
        <div className="flex items-center gap-2 flex-wrap text-[10px] font-semibold text-ink-muted">
          {requiresCamera && (
            <span className="bg-cream-subtle border border-cream-border/70 px-2 py-0.5 rounded-md">
              Caméra requise
            </span>
          )}
          {requiresVoiceover && (
      ...
      {recordedFeedback ? (
        ...
        <div className="flex flex-col gap-3 p-3.5 bg-cream-subtle/80 border border-primary-border/60 rounded-xl animate-fadeIn">
        ...
        <div className="flex flex-col gap-2 pt-1">
          ...
          {draftError && (
            <p className="text-xs text-red-600 font-medium pt-0.5">{draftError}</p>
          )}
          ...
          {modifyNoticeOpen && (
            <div className="bg-cream-subtle border border-cream-border p-2.5 rounded-xl text-xs text-ink-muted flex items-start justify-between gap-2">
              <p>
...
function renderFormatPreview(
  ...
  switch (format) {
        ...
        <div className="relative w-full h-36 sm:h-40 rounded-xl bg-ink text-white p-3 flex flex-col justify-between overflow-hidden shadow-xs border border-white/10">
          {media ? (
            <>
              ...
              />
              <div aria-hidden="true" className="absolute inset-0 bg-linear-to-t from-black/90 via-black/40 to-transparent" />
            </>
          ) : (
            <div aria-hidden="true" className="absolute -right-8 -bottom-8 w-32 h-32 rounded-full bg-primary/30 blur-xl pointer-events-none" />
          )}

          <div className="flex items-center justify-between text-[10px] font-bold text-primary-light relative z-10">
          ...
          <div className="my-auto flex items-center gap-3 relative z-10">
            <div className="w-10 h-10 rounded-full bg-primary text-white flex items-center justify-center text-sm shadow-md shrink-0">
              ▶
            </div>
            <div className="flex flex-col gap-0.5">
          ...
          <div className="flex items-center justify-between text-[9px] text-white/70 relative z-10">
            ...
            <span className="text-primary-light font-bold">✦ Dynamic Reel</span>
          </div>
        </div>
      )
    ...
    case 'INSTAGRAM_CAROUSEL':
      return (
        <div className="relative w-full h-36 sm:h-40 rounded-xl bg-cream-subtle p-3 flex flex-col justify-between overflow-hidden border border-cream-border">
          {/* Stacked Cards Background Layers */}
          <div className="absolute right-3 top-3 w-[88%] h-28 rounded-lg bg-cream-border/60 border border-cream-border transform rotate-2 pointer-events-none" />
          <div className="absolute right-5 top-2 w-[88%] h-28 rounded-lg bg-primary-light/50 border border-primary-border/40 transform -rotate-1 pointer-events-none" />

          {/* Top Main Slide */}
          <div className="relative z-10 w-[92%] h-full bg-white border border-cream-border rounded-lg p-3 flex flex-col justify-between shadow-xs overflow-hidden">
            {media && (
              <>
                <img
                  src={media.url}
                ...
                />
                <div aria-hidden="true" className="absolute inset-0 bg-linear-to-t from-white via-white/80 to-transparent" />
              </>
            )}
            <div className="flex items-center justify-between text-[10px] font-bold text-primary uppercase tracking-wider relative z-10">
              <span>📚 Carrousel Multi-Slides</span>
              <span className="bg-primary-light text-primary-dark px-1.5 py-0.5 rounded-md text-[9px]">
                1 / 4 slides
              </span>
            </div>
            ...
            <div className="flex items-center justify-between text-[9px] text-ink-muted relative z-10">
              ...
              <span className="text-primary font-semibold">✦ Mūza Stack</span>
            </div>
          </div>
        </div>
      )
    ...
    case 'INSTAGRAM_STORY':
      return (
        <div className="relative w-full h-36 sm:h-40 rounded-xl bg-linear-to-br from-primary-light to-white p-3 flex flex-col justify-between border-2 border-primary/40 overflow-hidden">
          {media && (
            <>
              <img
                src={media.url}
              ...
              />
              <div aria-hidden="true" className="absolute inset-0 bg-linear-to-t from-black/80 via-black/20 to-black/40" />
            </>
          )}
          <div className="flex items-center gap-2 relative z-10">
            <div className="w-6 h-6 rounded-full bg-primary text-white flex items-center justify-center text-[10px] font-bold ring-2 ring-primary-border">
              ✦
            </div>
          ...
          <div className="my-auto text-center px-2 relative z-10">
            ...
            <span className="inline-block mt-1 text-[9px] font-semibold text-primary bg-white/90 px-2 py-0.5 rounded-full border border-primary-border/30">
              Sondage / Sticker Interactif
            </span>
          </div>

          <div className={`text-[9px] font-bold text-center relative z-10 ${media ? 'text-white/90' : 'text-primary-dark'}`}>
    ...
    case 'WEBSITE_PAGE':
      return (
        <div className="relative w-full h-36 sm:h-40 rounded-xl bg-white p-3 flex flex-col justify-between border border-emerald-600/30 overflow-hidden shadow-xs">
          {media && (
            <>
              <img
                src={media.url}
              ...
              />
              <div aria-hidden="true" className="absolute inset-0 bg-linear-to-t from-white via-white/85 to-transparent" />
            </>
          )}
          <div className="flex items-center justify-between border-b border-cream-border pb-1.5 text-[10px] font-mono text-ink-muted relative z-10">
          ...
          <div className="flex items-center justify-between text-[9px] text-ink-muted pt-1 border-t border-cream-border/60 relative z-10">
        ...
        <div className="relative w-full h-36 sm:h-40 rounded-xl bg-blue-50/50 p-3 flex flex-col justify-between border border-blue-200 overflow-hidden">
          {media && (
            <>
              ...
              />
              <div aria-hidden="true" className="absolute inset-0 bg-linear-to-t from-blue-50 via-blue-50/80 to-transparent" />
            </>
          )}
    ...
    default:
      return (
        <div className="relative w-full h-36 sm:h-40 rounded-xl bg-linear-to-r from-white via-primary-light/30 to-white p-3 flex flex-col justify-between border border-cream-border overflow-hidden">
          {media && (
            <>
              <img
                src={media.url}
          ...
          <div className="flex items-center justify-between text-[10px] font-bold text-ink-muted uppercase relative z-10">
            ...
            <span className="text-primary">Aperçu Visuel</span>
          </div>
          ...
          <div className="my-auto flex items-center gap-3 relative z-10">
            {media ? (
              <img
                src={media.url}
                alt={media.alt}
                className="w-12 h-12 rounded-lg object-cover border border-cream-border shrink-0"
              />
            ) : (
              <div className="w-12 h-12 rounded-lg bg-primary-light border border-primary-border/40 flex items-center justify-center text-primary shrink-0 font-serif font-bold text-lg">
                ✦
              </div>
            )}
            <div className="flex flex-col gap-0.5">
          ...
          <div className="flex items-center justify-between text-[9px] text-ink-muted relative z-10">
            ...
            <span className="text-primary font-semibold">✦ Studio Mūza</span>
          </div>
        </div>
      )
  }
